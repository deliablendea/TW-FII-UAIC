...
class GoogleOAuthController {
    ...
    public function authorize() {
        ...
        $authUrl = GoogleConfig::getAuthUrl($state);
    ...
    public function callback() {
        ...
        if (!isset($_SESSION['oauth_state']) || $state !== $_SESSION['oauth_state']) {
            ...
            header('Location: ' . $baseUrl . '/TW-FII-UAIC/public/dashboard.html?oauth=error&reason=invalid_state');
            exit;
        }
        ...
        if (empty($code)) {
            ...
            header('Location: ' . $baseUrl . '/TW-FII-UAIC/public/dashboard.html?oauth=error&reason=no_code');
            exit;
        }
        ...
        if (!$tokenData['success']) {
            error_log("Token exchange failed: " . json_encode($tokenData));
            header('Location: ' . $baseUrl . '/TW-FII-UAIC/public/dashboard.html?oauth=error&reason=token_exchange');
            exit;
        }
        ...
        if ($result['success']) {
            ...
            header('Location: ' . $baseUrl . '/TW-FII-UAIC/public/dashboard.html?oauth=success');
        } else {
            error_log("Token save failed: " . json_encode($result));
            header('Location: ' . $baseUrl . '/TW-FII-UAIC/public/dashboard.html?oauth=error&reason=token_save');
        }
        exit;
    }
    ...
    public function getStatus() {
        ...
        $userId = $_SESSION['user_id'];
        $isConnected = $this->oauthTokenModel->isTokenValid($userId, 'google');
    ...
    public function disconnect() {
        ...
        $userId = $_SESSION['user_id'];
        $result = $this->oauthTokenModel->deleteToken($userId, 'google');
